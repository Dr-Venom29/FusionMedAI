...
- **Reasoning**: Decoupling follows the Single Responsibility Principle (SRP) (Martin, 2003) and Separation of Concerns (SoC) (Martin, 2017), making code easier to test, debug, and maintain. 

### 3. Dedicated Verification Scripts
...
- **Decision**: Store all preprocessing parameters inside the central configuration module (`src/config.py`).
- **Reasoning**: Adheres to the Don't Repeat Yourself (DRY) principle (Hunt & Thomas, 1999) by avoiding duplicated hardcoded settings across different files. 

### 5. Dynamic Verification of Split Sizes
...
- **Reasoning**: Retrieving sizes dynamically ensures the tests remain valid even if the split ratio is changed.
...
- **Reasoning**: Detecting invalid configurations before model training prevents wasting computational resources and simplifies debugging. Errors are reported immediately instead of failing after several training epochs.

### 7. Explicit Exceptions Instead of Assertions
...
### 10. Model-Agnostic Data Pipeline
- **Decision**: Design the pipeline to output standardized tensors without coupling it to any specific neural network.
- **Reasoning**: Ensures that the data loading pipeline is completely decoupled from the model architecture. Changing the classification backbone is achieved by modifying only the model instantiation code, while the data pipeline remains identical.

### 11. Separation of Data Pipeline From Training
- **Decision**: Structurally isolate files into distinct packages (`src/data/`, `src/models/`, `src/training/`) instead of a monolithic project structure.
- **Reasoning**: Isolating concerns ensures that modifications to the optimization schedule, loss functions, or evaluation metrics do not affect dataset loading or augmentation code.
