...
$$T^* = \arg\min_T -\frac{1}{N_{\text{val}}} \sum_{i=1}^{N_{\text{val}}} \log \left[ \text{softmax}\left( \frac{z_i}{T} \right) \right]_{y_i}$$
...
$$\{w^*, b^*\} = \arg\min_{w, b} -\frac{1}{N_{\text{val}}} \sum_{i=1}^{N_{\text{val}}} \log \left[ \text{softmax}(w \odot z_i + b) \right]_{y_i}$$
