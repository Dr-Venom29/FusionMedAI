...
## Decisions Adopted During Baseline Implementation & Benchmarking

During the implementation of the baseline deep learning model (Step 4) and architecture benchmarking (Step 5), several design choices were finalized to establish a stable and reproducible pipeline, drawing on the exploratory analysis.

As detailed in **Chapter 5**, the pipeline successfully integrated:
1. **$224 \times 224$ Input Resolution** to enable rapid training iterations and leverage ImageNet weights.
