...
def print_class_distributions(df_dict: dict) -> bool:
    ...
    try:
        ...
        from src.retina.config import RAW_DATA
        original_csv_path = RAW_DATA / "aptos2019" / "train.csv"
        if not original_csv_path.exists():
            print(f"[X] Original CSV not found at '{original_csv_path}' for distribution baseline.")
            return False
        ...
        print(f"[X] Failed to compute class distributions: {e}")
        return False
